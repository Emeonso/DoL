...
function clothingIdentity(slot, item) {
    if (!item) return undefined;
    return [slot, item.variable || item.name, item.modder || ""].join("::");
}

function findClothingByIdentity(slot, identity) {
    if (identity === undefined || identity === null) return undefined;
    return (setup.clothes?.[slot] || []).find(item => clothingIdentity(slot, item) === identity);
}

function findClothingByName(slot, name, modder) {
    const items = setup.clothes?.[slot] || [];
    return (
        items.find(item => item.name === name && (modder === undefined || item.modder === modder)) ||
        items.find(item => item.name === name)
    );
}

function getOutfitClothing(outfit, slot) {
    if (!outfit) return undefined;
    return findClothingByIdentity(slot, outfit.itemIds?.[slot]) || findClothingByName(slot, outfit[slot], outfit.modder);
}

function setOutfitClothing(outfit, slot, item) {
    if (!outfit || !item) return;
    if (!outfit.itemIds) outfit.itemIds = {};
    outfit[slot] = item.name;
    outfit.itemIds[slot] = clothingIdentity(slot, item);
}

function syncOutfitItemIds(outfit) {
    if (!outfit) return outfit;
    if (!outfit.itemIds) outfit.itemIds = {};
    for (const slot of setup.clothes_all_slots || []) {
        const item = getOutfitClothing(outfit, slot);
        if (item) setOutfitClothing(outfit, slot, item);
    }
    return outfit;
}

function syncAllOutfitItemIds() {
    if (Array.isArray(V.outfit)) V.outfit.forEach(outfit => syncOutfitItemIds(outfit));
}

function sameClothing(slot, first, second) {
    if (!first || !second) return first === second;
    const firstId = clothingIdentity(slot, first);
    const secondId = clothingIdentity(slot, second);
    return firstId && secondId ? firstId === secondId : first.name === second.name;
}

window.clothingIdentity = clothingIdentity;
window.findClothingByIdentity = findClothingByIdentity;
window.findClothingByName = findClothingByName;
window.getOutfitClothing = getOutfitClothing;
window.setOutfitClothing = setOutfitClothing;
window.syncOutfitItemIds = syncOutfitItemIds;
window.syncAllOutfitItemIds = syncAllOutfitItemIds;
window.sameClothing = sameClothing;

function exportOutfit(outfitIndex) {
	syncOutfitItemIds(V.outfit[outfitIndex]);
	const outfit = clone(V.outfit[outfitIndex]);
...
function wardrobeSyncStylePanelHeight() {
	const layout = document.querySelector(".wardrobe-top-layout");
	const paper = layout && layout.querySelector(".wardrobe-paperdoll-panel");
	const summary = layout && layout.querySelector(".wardrobe-summary-panel");
	const style = summary && summary.querySelector(".wardrobe-style-panel");
	if (!paper || !summary) return;

	const open = Boolean(style);
	summary.classList.toggle("wardrobe-style-panel-open", open);
	if (open) {
		summary.style.height = `${Math.ceil(paper.getBoundingClientRect().height)}px`;
	} else {
	...
	const stable = input => {
		...
		if (input && typeof input === "object") {
			...
			Object.keys(input).sort().forEach(key => {
				if (key !== "lastTaken" && key !== "previewBookkeeping" && key !== "wardrobePreview") result[key] = stable(input[key]);
			});
			return result;
		}
		return input;
...
window.wardrobePreviewComparable = window.wardrobePreviewComparable || wardrobePreviewComparable;

function wardrobePreviewEqual(a, b) {
	return window.wardrobePreviewComparable(a) === window.wardrobePreviewComparable(b);
}

window.wardrobePreviewEqual = window.wardrobePreviewEqual || wardrobePreviewEqual;

function wardrobePreviewCalculate(preview) {
	if (!preview || !preview.baseline || !preview.current) return 0;
	const slots = ["over_upper", "over_lower", "over_head", "upper", "lower", "under_upper", "under_lower", "genitals", "head", "face", "neck", "hands", "handheld", "legs", "feet"];
	let changes = slots.reduce((total, slot) => total + (window.wardrobePreviewEqual(preview.baseline.worn[slot], preview.current.worn[slot]) ? 0 : 1), 0);
	if (!window.wardrobePreviewEqual(preview.baseline.hair, preview.current.hair)) changes++;
	if (!window.wardrobePreviewEqual(preview.baseline.face, preview.current.face)) changes++;
	["lipstick", "eyeshadow", "mascara", "blusher", "concealer", "browscolour", "eyelenses"].forEach(key => {
		if (!window.wardrobePreviewEqual(preview.baseline.makeup[key], preview.current.makeup[key])) changes++;
	});
	return changes;
}

window.wardrobePreviewCalculate = window.wardrobePreviewCalculate || wardrobePreviewCalculate;

function wardrobeHairDisplayName(style) {
	return style.name_cap || String(style.name || "").replace(/(^|\s)\S/g, match => match.toUpperCase());
}

window.wardrobeHairDisplayName = window.wardrobeHairDisplayName || wardrobeHairDisplayName;

function wardrobeHairCategories(layer) {
	return layer === "fringe" ? [
		{id: "straight", label: "Straight", preview: "hime", variables: ["default", "blunt locks", "curtain", "emo", "emo left", "emo right", "flat", "framed", "hime", "loose", "middlepart", "overgrown", "sectioned", "short air vents", "side-pinned", "straight", "trident"]},
		{id: "curly-textured", label: "Curly", preview: "ringlets", variables: ["aerial", "drill ringlets", "front braids", "loose antenna", "messy curls", "milkmaid", "princess wave", "ringlets", "sideswept braid", "sidecut"]},
		{id: "other-distinctive", label: "Other / Distinctive Shapes", preview: "default", variables: ["bedhead", "bowl", "buzzcut", "dread bun", "dreadlocks", "fro", "messy", "mohawk", "parted", "quiff", "ruffled", "ringlet curl", "sleek", "split", "straight curl", "straight tails", "swept back", "swept left", "thin flaps", "tied back", "wide flaps"]},
		{id: "all", label: "All Styles", variables: null}
	] : [
		{id: "ponytails", label: "Ponytails", preview: "ponytail", variables: ["flat ponytail", "fluffy ponytail", "jellyfish ponytail", "ponytail", "ribbon tail", "sidetail left", "sidetail right", "thick ponytail", "thick sidetail", "half-up"]},
		{id: "twintails", label: "Twintails", preview: "thick twintails", variables: ["bubble tails", "crescent tails", "curly pigtails", "half up twintail", "jellyfish twintails", "low tails", "pigtails", "princess twintails", "sailor buns", "scorpion tails", "thick pigtails", "thick twintails", "twintails"]},
		{id: "volume", label: "Volume", preview: "afro puffs", variables: ["messy bun", "space buns", "fro", "afro pouf", "afro puffs", "dreads", "french bob", "layered bob", "straight bob"]},
		{id: "braids", label: "Braids", preview: "twin braids", variables: ["braid left", "braid right", "heart braid", "left fishtail", "loop braid", "right fishtail", "twin braids", "twin fishtails", "twin double braids"]},
		{id: "curly", label: "Curly", preview: "princess wave", variables: ["curl", "curly side up", "defined curl", "drill ringlets", "princess wave"]},
		{id: "straight", label: "Straight", preview: "straight", variables: ["straight", "sleek", "neat", "all down", "default", "loose"]},
		{id: "other", label: "Other", preview: "messy ponytail", variables: ["bedhead", "messy", "messy ponytail", "ruffled", "shaved", "short", "short spiky", "sidecut", "swept left"]},
		{id: "all", label: "All Styles", variables: null}
	];
}

window.wardrobeHairCategories = window.wardrobeHairCategories || wardrobeHairCategories;

function wardrobeHairMatches(style, category) {
	return category.id === "all" || category.variables.includes(style.variable);
}

window.wardrobeHairMatches = window.wardrobeHairMatches || wardrobeHairMatches;

function wardrobeHairIsSingleStyle(layer, variable) {
	const retained = setup.hairSingleStyleRetained?.[layer] || [];
	if (retained.includes(variable)) return false;
	return (setup.hairSingleStyles || []).some(style => style && style[layer] === variable);
}

window.wardrobeHairIsSingleStyle = window.wardrobeHairIsSingleStyle || wardrobeHairIsSingleStyle;
