...
/* Human named NPCs with explicit runtime references. Passage titles are not
 * a reliable source of truth: shared classes, jobs, and encounters use the
 * <<npc Name>> widget without prefixing the passage title with that name. */
const namedNpcPortraitNames = Object.freeze([
	"Avery", "Bailey", "Briar", "Charlie", "Darryl", "Doren", "Eden", "Gwylan",
	"Harper", "Jordan", "Kylar", "Landry", "Leighton", "Mason", "Morgan", "River",
	"Robin", "Sam", "Sirris", "Whitney", "Winter", "Niki", "Quinn", "Remy", "Alex",
	"Wren", "Sydney", "Zephyr",
]);

function namedNpcFromPassage(passageName) {
	return namedNpcPortraitNames
		.slice()
		.sort((left, right) => right.length - left.length)
		.find(name => {
			if (passageName === name) return true;
			if (!passageName.startsWith(name)) return false;
			const boundary = passageName[name.length];
			return boundary !== undefined && (/\s/.test(boundary) || boundary === "'" || boundary === "\u2019");
		}) || "";
}

function namedNpcFromActiveState() {
	const activeNames = Array.isArray(V.npc) ? V.npc.filter(name => typeof name === "string") : [];
	return activeNames.filter((name, index, names) => namedNpcPortraitNames.includes(name) && names.indexOf(name) === index);
}

function namedNpcPortraitTargets(passageName) {
	const activeNames = Array.isArray(V.npc) ? V.npc.filter(name => typeof name === "string") : [];
	const supportedActiveNames = namedNpcFromActiveState();
	/* If the passage executed any named-NPC widget, trust that state even when
	 * the active NPC is intentionally unsupported by the human portrait model. */
	if (activeNames.length > 0) return supportedActiveNames;
	const titleFallback = namedNpcFromPassage(passageName);
	return titleFallback ? [titleFallback] : [];
}
function namedNpcPortraitTooltipId(name) {
	return `named-npc-portrait-tooltip-${String(name).toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
}
window.namedNpcPortraitTooltipId = namedNpcPortraitTooltipId;

const namedNpcPortraitInfoFields = [
	["sex", "Sex"],
	["age", "Age"],
	["pronouns", "Pronouns"],
...
function namedNpcPortraitInfoNode(info, field) {
	return info?.querySelector(`[data-info-field="${field}"]`);
}

function ensureNamedNpcPortraitTooltipStructure(tooltip) {
	let header = tooltip.querySelector(".named-npc-portrait-tooltip-header");
	if (!header) {
		header = document.createElement("span");
		header.className = "named-npc-portrait-tooltip-header";
		const label = document.createElement("b");
		label.className = "named-npc-portrait-tooltip-header-label";
		label.textContent = "IDENTIFICATION CARD";
		header.append(label);
		tooltip.prepend(header);
	}
	let body = tooltip.querySelector(".named-npc-portrait-tooltip-body");
	if (!body) {
		body = document.createElement("span");
		body.className = "named-npc-portrait-tooltip-body";
		tooltip.append(body);
	}
	let canvasTarget = body.querySelector(".named-npc-portrait-tooltip-canvas");
	if (!canvasTarget) {
		canvasTarget = document.createElement("span");
		canvasTarget.className = "named-npc-portrait-tooltip-canvas";
		body.prepend(canvasTarget);
	}
	let info = body.querySelector(".named-npc-portrait-info");
	if (!info) {
		info = document.createElement("span");
		info.className = "named-npc-portrait-info";
		const name = document.createElement("strong");
		name.className = "named-npc-portrait-info-name";
		name.dataset.infoField = "name";
		const species = document.createElement("small");
		species.className = "named-npc-portrait-info-species";
		species.dataset.infoField = "species";
		const grid = document.createElement("span");
		grid.className = "named-npc-portrait-info-grid";
		for (const [field, labelText] of namedNpcPortraitInfoFields) {
			const label = document.createElement("span");
			label.className = "named-npc-portrait-info-label";
			label.textContent = labelText;
			const value = document.createElement("span");
			value.className = "named-npc-portrait-info-value";
			value.dataset.infoField = field;
			grid.append(label, value);
		}
		info.append(name, species, grid);
		body.append(info);
	}
	let message = tooltip.querySelector(".named-npc-portrait-tooltip-message");
	if (!message) {
		message = document.createElement("span");
		message.className = "named-npc-portrait-tooltip-message";
		message.setAttribute("aria-live", "polite");
		message.hidden = true;
		tooltip.append(message);
	}
	return { canvasTarget, info, message };
}

function closeNamedNpcPortraitTooltip(tooltip) {
	if (!tooltip) return;
	tooltip.hidden = true;
	const button = tooltip.parentElement?.querySelector(".named-npc-portrait-button");
	if (button) button.setAttribute("aria-expanded", "false");
	const canvasTarget = tooltip.querySelector(".named-npc-portrait-tooltip-canvas");
	if (canvasTarget) canvasTarget.replaceChildren();
	const message = tooltip.querySelector(".named-npc-portrait-tooltip-message");
	if (message) {
		message.hidden = true;
		message.textContent = "";
	}
...
function closeNamedNpcPortraitTooltips() {
	document.querySelectorAll(".named-npc-portrait-tooltip:not([hidden])").forEach(closeNamedNpcPortraitTooltip);
}

function toggleNamedNpcPortraitTooltip(button) {
	const relationName = button.closest(".relation-name");
	const tooltipId = button.getAttribute("aria-controls");
	let tooltip = relationName?.querySelector(".named-npc-portrait-tooltip") || (tooltipId ? document.getElementById(tooltipId) : null);
	if (!tooltip && relationName) {
		tooltip = document.createElement("span");
		tooltip.className = "named-npc-portrait-tooltip";
		tooltip.setAttribute("role", "tooltip");
		tooltip.hidden = true;
		relationName.append(tooltip);
	}
	if (!tooltip) return;
	const wasOpen = button.getAttribute("aria-expanded") === "true" && !tooltip.hidden;
	closeNamedNpcPortraitTooltips();
	if (wasOpen) return;

	const name = button.getAttribute("data-npc-portrait-name") || button.textContent.trim();
	const structure = ensureNamedNpcPortraitTooltipStructure(tooltip);
	const details = window.NamedNpcPortrait?.detailsFor?.(name);
	if (details) {
		const nameNode = namedNpcPortraitInfoNode(structure.info, "name");
		const speciesNode = namedNpcPortraitInfoNode(structure.info, "species");
		if (nameNode) nameNode.textContent = details.name;
		if (speciesNode) speciesNode.textContent = details.species;
		for (const field of namedNpcPortraitInfoFields.map(([key]) => key)) {
			const node = namedNpcPortraitInfoNode(structure.info, field);
			if (node) node.textContent = details[field] || "Unknown";
		}
	}
	structure.canvasTarget.replaceChildren();
	structure.message.hidden = true;
	structure.message.textContent = "";
	tooltip.hidden = false;
	button.setAttribute("aria-expanded", "true");
	let rendered = false;
	try {
		rendered = window.NamedNpcPortrait?.render(structure.canvasTarget, name) === true;
	} catch (error) {
		console.warn(name + " Social portrait render failed", error);
	}
	if (!rendered) {
		structure.message.textContent = V.options?.images === 1 ? "Portrait unavailable." : "Portraits are disabled in image settings.";
		structure.message.hidden = false;
	}
}
window.toggleNamedNpcPortraitTooltip = toggleNamedNpcPortraitTooltip;

if (!window.__namedNpcSocialPortraitHandlers) {
	window.__namedNpcSocialPortraitHandlers = true;
	document.addEventListener("click", function (ev) {
		const target = ev.target;
		const button = target instanceof Element ? target.closest(".named-npc-portrait-button") : null;
		if (button) {
			ev.preventDefault();
			toggleNamedNpcPortraitTooltip(button);
			return;
		}
		if (!(target instanceof Element) || !target.closest(".named-npc-portrait-tooltip")) closeNamedNpcPortraitTooltips();
	}, true);
	document.addEventListener("keydown", function (ev) {
		if (ev.key === "Escape" || ev.keyCode === 27) closeNamedNpcPortraitTooltips();
	});
}$(document).on(":passagerender", function (ev) {
	const passageName = State.passage || "";
	const portraitNpcs = namedNpcPortraitTargets(passageName);
	const content = $(ev.content);
	closeNamedNpcPortraitTooltips();
	content.find(".named-npc-portrait, .robin-portrait, .robin-hook-diagnostic").remove();
	if (portraitNpcs.length > 0 && window.NamedNpcPortrait) {
		/* Prepend in reverse so multiple active NPCs retain the widget order. */
		for (const portraitNpc of portraitNpcs.slice().reverse()) {
			...
			try {
				if (!window.NamedNpcPortrait.render(portrait, portraitNpc)) portrait.remove();
			} catch (error) {
				portrait.remove();
				console.warn(portraitNpc + " portrait render failed", error);
			}
		}
	}
...
function settingsDisableElement() {
	$(() => {
		$("[data-disabledif]").each(function () {
			...
			[orig.data("target")].flat().forEach(e => $("[name$='" + Util.slugify(e) + "']").off("click.dolSettingsDisable").on("click.dolSettingsDisable", updateButtonsActive));
			if (disabledif) {
				updateButtonsActive();
			}
		});
...
function onInputChanged(func, bindingKey = "default") {
	if (!func || typeof func !== "function") return;
	$(() => {
		const root = $("#customOverlayContent").length ? $("#customOverlayContent") : $("#passages > .passage");
		if (!root.length) return;
		const namespace = ".dolInputChanged" + String(bindingKey).replace(/[^a-zA-Z0-9_]/g, "_");
		root.off("change" + namespace, "input, select").on("change" + namespace, "input, select", function () {
			func();
		});
	});
}
