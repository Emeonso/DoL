...
function setKnowsAboutPregnancyCurrentLoaded() {
	...
	if (playerIsPregnant() && playerBellyVisible(true)) {
		const namedNPCs = new Set(V.NPCNameList);
		for (const npc of V.NPCList) {
			if (npc?.fullDescription && namedNPCs.has(npc.fullDescription)) setKnowsAboutPregnancy("pc", npc.fullDescription);
		}
	}
}
...
function childrenByBirth() {
	const source = V.children;
	const version = T.dolChildrenByBirthVersion || 0;
	const cached = T.dolChildrenByBirthCache;
	if (cached?.source === source && cached.version === version) return cached.map;
	...
	T.dolChildrenByBirthCache = { source, version, map };
	return map;
}
