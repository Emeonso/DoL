...
const combatActionColours = {
	Default: {
		brat: [
			...
			/* mouthaction */
			"pullaway", "ejacspit", "pullawayvagina", "finish", "novaginal", "nopenile", "noanal", "scream", "mock", "breastclosed", "breastpull", "pullawaykiss", "noupper", "analpull", "up", "stifleorgasm", "stifle", "mouthresistW", "handcloseW", "growl", "askPullOut", "haggle", "disparage",
			/* penisaction */
...
function combatButtonAdjustments(name, extra) {
	jQuery(document).off("change.dolCombatAction", "#listbox-" + name).on("change.dolCombatAction", "#listbox-" + name, { name, extra }, function (e) {
		/* console.log(e.data); */
...
function combatDefaults() {
	jQuery(document).off("change.dolCombatDefaults", "#listbox--defaultoption").on("change.dolCombatDefaults", "#listbox--defaultoption", function (e) {
...
/* Consensual player-agency relations. The registry is authoritative for the
 * new actions; DefaultActions continues to choose only the highlighted action. */
/* Consensual NPC dynamics for 0.6.2.2.  This is deliberately encounter-local. */
const combatDynamicsConfig = Object.freeze({
	version: "0.6.2.2-candidate-model-2",
	pleasureBase: Object.freeze({ vaginal: 3, anal: 4, oral: 2, hand: 2, "genital-rub": 2, other: 2 }),
	lowMax: 50,
	acceptableMax: 100,
	exceedsMax: 180,
	maxHistory: 12,
...
function combatDynamicsClamp(value, minimum, maximum) {
	return Math.max(minimum, Math.min(maximum, Number(value) || 0));
}

function combatDynamicsDebugPercent(value, minimum, maximum) {
 const numeric = Number(value) || 0;
 const low = Number(minimum) || 0;
 const high = Number(maximum);
 if (!Number.isFinite(high) || high <= low) return 0;
 return Math.round(Math.clamp((numeric - low) / (high - low) * 100, 0, 100));
}
function combatDynamicsDebugEnabled() {
	return V.debug === 1 || V.debug === true;
}
...
function combatDynamicsNpcAnatomy(npc) {
	const penis = typeof combatAgencyAnatomy === "function" && combatAgencyAnatomy(npc, "penis");
	const vagina = typeof combatAgencyAnatomy === "function" && combatAgencyAnatomy(npc, "vagina");
	return { penis, vagina, sex: penis && vagina ? "both" : penis ? "penis" : vagina ? "vagina" : "unknown" };
}

function combatDynamicsPlayerTargetAvailable(target) {
	switch (target) {
		case "mouth": return typeof combatAgencyFaceOpen !== "function" || combatAgencyFaceOpen();
		case "vagina": return typeof combatAgencyPlayerHas !== "function" || (combatAgencyPlayerHas("vagina") && combatAgencyPlayerUncovered("vagina"));
		case "penis": return (!!V.player?.penisExist || (typeof playerHasStrapon === "function" && playerHasStrapon())) && (typeof combatAgencyPlayerUncovered !== "function" || combatAgencyPlayerUncovered("penis"));
		case "anus": return typeof combatAgencyPlayerHas !== "function" || (combatAgencyPlayerHas("anus") && combatAgencyPlayerUncovered("anus"));
		case "hands": return ![V.leftarm, V.rightarm].every(value => ["bound", "grappled", "behind"].includes(value));
		case "feet": return !["bound", "legLock", "legLocked"].includes(V.feetuse);
		case "chest":
		case "thighs":
		case "buttocks":
			return true;
...
function combatDynamicsCandidateDefinitions(npc) {
	const anatomy = combatDynamicsNpcAnatomy(npc);
	const definitions = [];
	if (anatomy.vagina) definitions.push(
		{ id: "vagina-mouth", className: "oral", target: "mouth", nativeStates: ["mouthentrance", "mouthimminent", "mouth"] },
		{ id: "vagina-hands", className: "hand", target: "hands", nativeStates: ["leftarm", "rightarm", "botharm"] },
		{ id: "vagina-feet", className: "other", target: "feet", nativeStates: ["feet"] },
		{ id: "vagina-vagina", className: "vaginal", target: "vagina", nativeStates: ["vagina"] },
		{ id: "vagina-penis", className: "other", target: "penis", nativeStates: ["penis"] },
		{ id: "vagina-penis-alternate", className: "other", target: "penis", nativeStates: ["otheranus"] },
	);
	if (anatomy.penis) definitions.push(
		{ id: "penis-vagina", className: "vaginal", target: "vagina", nativeStates: ["vaginaentrance", "vaginaimminent", "vagina"] },
		{ id: "penis-anus", className: "anal", target: "anus", nativeStates: ["anusentrance", "anusimminent", "anus"] },
		{ id: "penis-mouth", className: "oral", target: "mouth", nativeStates: ["mouthentrance", "mouthimminent", "mouth"] },
		{ id: "penis-hands", className: "hand", target: "hands", nativeStates: ["leftarm", "rightarm", "botharm"] },
		{ id: "penis-feet", className: "other", target: "feet", nativeStates: ["feet"] },
		{ id: "penis-chest", className: "other", target: "chest", nativeStates: ["chest"] },
		{ id: "penis-thighs", className: "other", target: "thighs", nativeStates: ["thighs"] },
		{ id: "penis-buttocks", className: "other", target: "buttocks", nativeStates: ["cheeks"] },
		{ id: "penis-penis", className: "other", target: "penis", nativeStates: ["penisentrance", "penisimminent", "penis"] },
	);
	return { anatomy, definitions };
}

function combatDynamicsCandidates(npc) {
	const { anatomy, definitions } = combatDynamicsCandidateDefinitions(npc);
	return definitions.filter(candidate => combatDynamicsPlayerTargetAvailable(candidate.target)).map(candidate => ({
		...candidate,
		sourceSex: candidate.id.startsWith("vagina-") ? "vagina" : "penis",
		score: 20,
		viable: true,
...
function combatDynamicsActiveCandidate(profile, index) {
	...
	const sourceSex = profile?.anatomicalSex === "vagina" ? "vagina" : profile?.anatomicalSex === "penis" ? "penis" : npc.penis && npc.penis !== 0 ? "penis" : "vagina";
	const nativePart = sourceSex === "penis" ? npc.penis : npc.vagina;
	const state = String(nativePart || "");
	return profile?.candidates?.find(candidate => candidate.sourceSex === sourceSex && candidate.nativeStates?.includes(state)) || null;
}

function combatDynamicsProfile(index) {
	const npc = V.NPCList[index] || {};
	const anatomy = combatDynamicsNpcAnatomy(npc);
	...
	const expectedThreshold = combatDynamicsConfig.thresholds.expected;
	const highThreshold = combatDynamicsConfig.thresholds.high;
	return {
		index,
		identity: npc.fullDescription || npc.description || `NPC ${index}`,
		anatomicalSex: anatomy.sex,

		thresholds: { low: lowThreshold, expected: expectedThreshold, high: highThreshold },
		candidates,
		anchor: viable.find(candidate => candidate.className === "vaginal")?.id || viable[0]?.id || "other",
		nextIntent: viable.find(candidate => candidate.className === "vaginal")?.id || viable[0]?.id || "other",
		lastDecision: "encounter start profile",
		lastClass: null,
		lastDelta: 0,
		lastBand: "unmeasured",
	};
...
function combatDynamicsState() {
	return V.consensualNpcDynamics && typeof V.consensualNpcDynamics === "object" ? V.consensualNpcDynamics : null;
}

function combatDynamicsInit() {
	if (!combatDynamicsIsEligible()) {
		V.consensualNpcDynamics = 0;
		return false;
	}
	const count = Math.min(Number(V.enemynomax || V.enemyno || 0), V.NPCList.length);
	const profiles = [];
	for (let index = 0; index < count; index++) profiles.push(combatDynamicsProfile(index));
	V.consensualNpcDynamics = {
		version: combatDynamicsConfig.version,
		enabled: true,
		eligible: true,
		turn: 0,
		turnOpen: false,
		turnStartArousal: Number(V.enemyarousal) || 0,
		currentNpc: 0,
		currentIntent: null,
		lastInteraction: null,
		pendingProposal: null,
		fallbackResult: null,
		lastDecision: "profile initialised",
		lastTransitionReason: "encounter start",
		lastAcceptance: null,
...
function combatDynamicsEnsure() {
	if (!combatDynamicsIsEligible()) return false;
	const state = combatDynamicsState();
	if (!state || state.version !== combatDynamicsConfig.version || !Array.isArray(state.profiles)) return combatDynamicsInit();
	if (state.validationDirty === false) return true;
	if (!Number.isFinite(Number(state.intentCooldown))) state.intentCooldown = 0;
	for (const profile of state.profiles) {
		if (profile && typeof profile === "object") {
			profile.thresholds = { low: combatDynamicsConfig.thresholds.low, expected: combatDynamicsConfig.thresholds.expected, high: combatDynamicsConfig.thresholds.high };
			if (Array.isArray(profile.candidates)) {
				for (const candidate of profile.candidates) {
					candidate.score = Math.max(0, Number(candidate.score) || 0);
					candidate.viable = candidate.score > 0;
					candidate.reason = candidate.viable ? "score above zero" : "score depleted";
				}
			}
...
function combatDynamicsBeginTurn() {
	if (!combatDynamicsEnsure()) return false;
	const state = combatDynamicsState();
	if (!state.enabled || state.turnOpen) return true;
	for (const profile of state.profiles) {
		const anchorCandidate = profile?.candidates?.find(candidate => candidate.id === profile.anchor);
		if (anchorCandidate && Number(anchorCandidate.score) > 0) anchorCandidate.score = (Number(anchorCandidate.score) || 0) + 1;
		if (anchorCandidate) {
			anchorCandidate.viable = anchorCandidate.score > 0;
			anchorCandidate.reason = anchorCandidate.viable ? "score above zero" : "score depleted";
		}
	}
	state.turn += 1;
	state.turnOpen = true;
	state.turnStartArousal = Number(V.enemyarousal) || 0;
	state.lastInteraction = null;
			return true;
}

function combatDynamicsInferClass() {
	const state = combatDynamicsState();
	const interaction = state?.lastInteraction;
	if (interaction?.class) return interaction.class;
	if (V.anususe === "penis" || V.anususe === "othermouth" || V.anususe === "otheranus") return "anal";
	if (["penis", "othervagina", "othermouth"].includes(V.vaginause) || ["penis", "othervagina", "othermouth"].includes(V.penisuse)) return "vaginal";
	if (["penis", "othervagina", "facesit", "otheranus"].includes(V.mouthuse)) return "oral";
	if (["leftarm", "rightarm", "otherhand", 1].includes(V.leftarm) || ["leftarm", "rightarm", "otherhand", 1].includes(V.rightarm)) return "hand";
	return "other";
}

function combatDynamicsIntentMatchesClass(intentClass, actualClass) {
	return intentClass === actualClass || (intentClass === "genital-rub" && actualClass === "other");
}

function combatDynamicsIntentChangeChance(profile, activeCandidate, desiredCandidate) {
	const L = Math.max(0, Number(desiredCandidate?.score) || 0);
	const rawS = Math.max(0, Number(activeCandidate?.score) || 0);
	// Safety: S must never exceed L, otherwise D would become negative.
	const S = Math.min(rawS, L);
	if (!desiredCandidate || L <= 0 || activeCandidate?.id === desiredCandidate.id) return { chance: 0, weight: 0, D: 0, S, L };
	const weightByBand = { poor: 1, average: 0.5, exceeds: 0.25, overwhelming: 0 };
	const W = Object.prototype.hasOwnProperty.call(weightByBand, profile?.lastBand) ? weightByBand[profile.lastBand] : 1;
	const D = Math.max(0, 1 - (S / L));
	const chance = Math.max(0, W - 0.75) + Math.min(W, 0.75) * D;
	return { chance: Math.max(0, Math.min(1, chance)) * 100, weight: W, D, S, L };
}
function combatDynamicsFinishTurn() {
	if (!combatDynamicsEnsure()) return null;
	const state = combatDynamicsState();
	if (!state.enabled || !state.turnOpen) return state?.last;
	const before = Number(state.turnStartArousal) || 0;
	const after = Number(V.enemyarousal) || 0;
	const delta = after - before;
	const profile = state.profiles[state.currentNpc] || state.profiles[0];
	const thresholds = profile?.thresholds || { low: combatDynamicsConfig.thresholds.low, expected: combatDynamicsConfig.thresholds.expected, high: combatDynamicsConfig.thresholds.high };
	const band = delta <= combatDynamicsConfig.lowMax ? "poor" : delta <= combatDynamicsConfig.acceptableMax ? "average" : delta <= combatDynamicsConfig.exceedsMax ? "exceeds" : "overwhelming";
	const className = combatDynamicsInferClass();
	const suggestedClass = state.currentIntent?.className;
	const suggestionAccepted = state.intentCooldown <= 0 && suggestedClass && combatDynamicsIntentMatchesClass(suggestedClass, className);
	if (suggestionAccepted) {
		state.intentCooldown = 3;
		state.lastAcceptance = `intent accepted: ${suggestedClass}`;
		state.lastTransitionReason = `native action matched ${suggestedClass} suggestion`;
	}
	const evaluatedCandidate = combatDynamicsActiveCandidate(profile, state.currentNpc) || profile?.candidates?.find(candidate => candidate.className === className);
	const satisfactionDelta = combatDynamicsConfig.satisfactionDeltas[band] || 0;
	if (evaluatedCandidate) {
		evaluatedCandidate.score = Math.max(0, (Number(evaluatedCandidate.score) || 0) + satisfactionDelta);
		evaluatedCandidate.viable = evaluatedCandidate.score > 0;
		evaluatedCandidate.reason = evaluatedCandidate.viable ? "score above zero" : "score depleted";
		evaluatedCandidate.poorTurns = band === "poor" ? (Number(evaluatedCandidate.poorTurns) || 0) + 1 : Number(evaluatedCandidate.poorTurns) || 0;
	}
	if (profile) {
		profile.lastClass = className;
		profile.lastDelta = delta;
		profile.lastBand = band;
		const nextCandidate = profile.candidates.filter(candidate => candidate.viable).sort((left, right) => right.score - left.score)[0];
		profile.nextIntent = nextCandidate?.id || null;
		profile.lastDecision = `${band} satisfaction: ${className} score ${satisfactionDelta >= 0 ? "+" : ""}${satisfactionDelta}; highest desire ${profile.nextIntent || "none"}`;
		state.lastTransitionReason = `${band} satisfaction after completed ${className} turn`;
	}
	state.last = { turn: state.turn, npc: state.currentNpc, before, after, delta, className, band, thresholds: { lowMax: combatDynamicsConfig.lowMax, acceptableMax: combatDynamicsConfig.acceptableMax, exceedsMax: combatDynamicsConfig.exceedsMax } };
	state.history.push(state.last);
	if (state.history.length > combatDynamicsConfig.maxHistory) state.history.shift();
	state.turnOpen = false;
	return state.last;
}

function combatDynamicsPrepareNpcAction(index) {
	if (!combatDynamicsEnsure()) return false;
	const state = combatDynamicsState();
	if (!state.enabled) return false;
	state.currentNpc = Number(index) || 0;
	const profile = state.profiles[state.currentNpc] || state.profiles[0];
	if (!profile) return false;
	const forceQueued = !!state.forceNext;
	if (state.intentCooldown > 0 && !forceQueued && state.currentIntent && Number(state.currentIntent.npc) === state.currentNpc) {
		state.intentCooldown -= 1;
		state.currentIntent = { ...state.currentIntent, source: "intent cooldown" };
		state.pendingProposal = { npc: state.currentNpc, desired: state.currentIntent.candidateId || state.currentIntent.className, source: "intent cooldown", status: "accepted", cooldown: state.intentCooldown };
		state.lastDecision = `holding accepted intent: ${state.currentIntent.candidateId || state.currentIntent.className} (${state.intentCooldown} turns remaining)`;
		return true;
	}
	const forcedCandidate = state.forceNext ? profile.candidates.find(candidate => candidate.id === state.forceNext || candidate.className === state.forceNext) : null;
	const desiredCandidate = forcedCandidate || profile.candidates.filter(candidate => candidate.viable).sort((left, right) => right.score - left.score)[0];
	const desired = desiredCandidate?.className || "other";
	const forced = forceQueued;
	if (forced) state.intentCooldown = 0;
	state.forceNext = null;
	const activeCandidate = combatDynamicsActiveCandidate(profile, state.currentNpc);
	const activeClass = activeCandidate?.className || combatDynamicsInferClass();
	if (!forced && desiredCandidate && activeCandidate?.id !== desiredCandidate.id) {
		const decision = combatDynamicsIntentChangeChance(profile, activeCandidate, desiredCandidate);
		const roll = Math.random() * 100;
		state.intentDecision = { active: activeCandidate?.id || activeClass, desired: desiredCandidate.id, S: decision.S, L: decision.L, D: decision.D, weight: decision.weight, chance: decision.chance, roll, passed: roll < decision.chance, cooldown: state.intentCooldown };
		if (roll >= decision.chance) {
			state.currentIntent = null;
			state.pendingProposal = { npc: state.currentNpc, desired: desiredCandidate.id, source: "weighted intent roll", status: "withheld", chance: decision.chance, roll };
			state.lastDecision = `intent withheld: ${desiredCandidate.id} (${Math.round(decision.chance)}%); native action remains in control`;
			state.lastTransitionReason = "weighted intent roll failed";
			return true;
		}
		state.lastDecision = `weighted intent passed: ${desiredCandidate.id} (${Math.round(decision.chance)}%)`;
	}
	if (!forced && (!desiredCandidate || activeCandidate?.id === desiredCandidate.id)) {
		state.currentIntent = null;
		state.pendingProposal = { npc: state.currentNpc, desired: desiredCandidate?.id || "none", source: "weighted intent roll", status: "already active" };
		state.lastDecision = `intent already active: ${desiredCandidate?.id || "none"}`;
		return true;
	}
	state.currentIntent = { npc: state.currentNpc, candidateId: desiredCandidate?.id || desired, className: desired, source: forced ? "debug force" : "satisfaction loop", sourceBodypart: desired === "oral" ? "mouth" : desired === "hand" ? "hand" : desired === "anal" ? "penis" : desired === "vaginal" ? "penis" : "native", targetBodypart: desired === "oral" ? "player mouth" : desired === "hand" ? "player hands" : desired === "anal" ? "player anus" : desired === "vaginal" ? "player vagina" : desiredCandidate?.target || "native" };
	state.pendingProposal = { npc: state.currentNpc, desired: desiredCandidate?.id || desired, source: state.currentIntent.source, status: "pending" };
	return true;
}function combatDynamicsApplyDesiredTransition(index) {
	if (!combatDynamicsEnsure()) return false;
	const state = combatDynamicsState();
	const intent = state?.currentIntent;
	if (!state.enabled || !intent || Number(intent.npc) !== Number(index) || !["satisfaction loop", "debug force"].includes(intent.source)) return false;
	const npc = V.NPCList[index];
	if (!npc) return false;
	const desired = intent.className;
	const current = String(npc.penis || "");
	const mouthStates = ["mouth", "mouthentrance", "mouthimminent", "mouthotheranus"];
	const vaginalStates = ["vagina", "vaginaentrance", "vaginaimminent", "vaginadouble"];
	const analStates = ["anus", "anusentrance", "anusimminent", "anusdouble"];
	let changed = false;
	if (desired !== "oral" && mouthStates.includes(current)) {
		npc.penis = 0;
		V.mouthuse = 0;
		V.mouthstate = 0;
		V.mouthtarget = 0;
		if (npc.location) npc.location.genitals = 0;
		changed = true;
	}
	if (desired !== "vaginal" && vaginalStates.includes(current)) {
		npc.penis = 0;
		V.vaginause = 0;
		V.vaginastate = 0;
		V.vaginatarget = 0;
	...
	if (desired !== "anal" && analStates.includes(current)) {
		npc.penis = 0;
		V.anususe = 0;
		V.anusstate = 0;
		V.anustarget = 0;
	...
	/* The native widget owns non-genital states too. Release an active chest
	 * interaction when a different candidate is explicitly suggested, otherwise
	 * penisinit never gets a chance to evaluate the new intent. */
	const candidate = state.profiles?.[state.currentNpc]?.candidates?.find(item => item.id === intent.candidateId);
	if (candidate && current === "chest" && !candidate.nativeStates.includes(current)) {
		npc.penis = 0;
		V.chestuse = 0;
		V.cheststate = 0;
		V.chesttarget = 0;
	...
	if (changed) state.lastTransitionReason = `released ${current} for ${desired} satisfaction-loop intent`;
	return changed;
}
...
function combatDynamicsClassForRelation(relation) {
	if (["agencyPenisToMouth", "agencyVaginaToMouth"].includes(relation)) return "oral";
	if (relation === "agencyPenisToVagina") return "vaginal";
	if (relation === "agencyPenisToAnus") return "anal";
	if (String(relation).startsWith("direct")) return "hand";
	return "other";
}

function combatDynamicsMarkRelation(relation, target) {
	if (!combatDynamicsEnsure()) return false;
	const state = combatDynamicsState();
	const className = combatDynamicsClassForRelation(relation);
	state.currentNpc = Number(target) || 0;
	state.lastInteraction = { class: className, relation, target: state.currentNpc, source: relation, turn: state.turn, sourceBodypart: combatAgencyRelation(relation)?.sourceBodypart || "player hand", targetBodypart: combatAgencyRelation(relation)?.targetBodypart || "native" };
	state.currentIntent = { npc: state.currentNpc, className, source: "accepted player transition", sourceBodypart: state.lastInteraction.sourceBodypart, targetBodypart: state.lastInteraction.targetBodypart };
	state.pendingProposal = { npc: state.currentNpc, desired: className, source: relation, status: "accepted" };
	state.fallbackResult = null;
	state.lastDecision = `accepted transition: ${relation}`;
	state.lastAcceptance = "accepted";
	return true;
}

function combatDynamicsActiveClass(explicitClass) {
	if (["vaginal", "anal", "oral", "hand", "genital-rub", "other"].includes(explicitClass)) return explicitClass;
	const state = combatDynamicsState();
	return state?.lastInteraction?.class || state?.currentIntent?.className || null;
}

function combatDynamicsPleasureAmount(nativeAmount, target, npcIndex, explicitClass) {
	if (!combatDynamicsIsEligible() || !combatDynamicsEnsure()) return nativeAmount;
	const state = combatDynamicsState();
	if (!state.enabled) return nativeAmount;
	const className = combatDynamicsActiveClass(explicitClass);
	if (!className) return nativeAmount;
	const multiplier = combatDynamicsConfig.pleasureBase[className] || combatDynamicsConfig.pleasureBase.other;
	const amount = Number(nativeAmount) || 0;
	const adjusted = Math.floor(amount * multiplier / 2);
	
	return adjusted;
}


function combatDynamicsRecordProposal(relation, target, status) {
	if (!combatDynamicsEnsure()) return false;
	const state = combatDynamicsState();
	const npc = Number(target) || 0;
	const className = combatDynamicsClassForRelation(relation);
	state.currentNpc = npc;
	state.pendingProposal = { npc, desired: className, source: relation, status };
	state.lastAcceptance = status;
	if (status === "refused") {
		state.lastTransitionReason = "proposal refused; native refusal consequences retained";
		state.fallbackResult = { npc, requested: className, fallback: state.profiles[npc]?.lastClass || state.profiles[npc]?.anchor || "current", reason: "proposal refused" };
	}
	return true;
}

function combatDynamicsAcceptanceOverride(target) {
	if (!combatDynamicsIsEligible() || !combatDynamicsEnsure()) return null;
	const state = combatDynamicsState();
	const override = state.acceptanceOverride;
	state.acceptanceOverride = null;
	if (override === "accept" || override === "refuse") {
		state.currentNpc = Number(target) || 0;
		state.lastAcceptance = override === "accept" ? "debug accepted" : "debug refused";
		state.pendingProposal = { npc: state.currentNpc, desired: state.currentIntent?.className || null, source: "debug control", status: override };
		return override;
	}
	return null;
}

function combatDynamicsDebugCommand(command, value) {
	if (!combatDynamicsIsEligible()) return false;
	combatDynamicsEnsure();
	const state = combatDynamicsState();
	if (!state) return false;
	switch (command) {
		case "reinit":
			return combatDynamicsInit();
		case "reroll":
			return combatDynamicsInit();
		case "clear":
			state.history = [];
			return true;
		case "force":
			state.forceNext = value || "vaginal";
			state.lastDecision = `debug force queued: ${state.forceNext}`;
			return true;
		case "accept":
			state.acceptanceOverride = "accept";
			return true;
		case "refuse":
			state.acceptanceOverride = "refuse";
			return true;
		case "arousal":
			V.enemyarousal = combatDynamicsClamp((Number(V.enemyarousal) || 0) + (Number(value) || 0), 0, Number(V.enemyarousalmax) || 500);
			return true;
...
function combatDynamicsDebugSnapshot() {
	...
	return {
		...
		thresholds: state?.profiles?.[state.currentNpc]?.thresholds || { low: combatDynamicsConfig.thresholds.low, expected: combatDynamicsConfig.thresholds.expected, high: combatDynamicsConfig.thresholds.high },
		profiles: state?.profiles || [],
		npcs,
		playerSkills: { oral: V.oralskill, hand: V.handskill, vaginal: V.vaginalskill, anal: V.analskill, seduction: typeof currentSkillValue === "function" ? currentSkillValue("seductionskill") : undefined },
		};
}

window.combatDynamicsDebugPercent = combatDynamicsDebugPercent;
window.combatDynamicsIsEligible = combatDynamicsIsEligible;
window.combatDynamicsInit = combatDynamicsInit;
window.combatDynamicsEnsure = combatDynamicsEnsure;
window.combatDynamicsBeginTurn = combatDynamicsBeginTurn;
window.combatDynamicsFinishTurn = combatDynamicsFinishTurn;
window.combatDynamicsPrepareNpcAction = combatDynamicsPrepareNpcAction;
window.combatDynamicsPreferredRoll = combatDynamicsPreferredRoll;
window.combatDynamicsApplyDesiredTransition = combatDynamicsApplyDesiredTransition;
window.combatDynamicsPleasureAmount = combatDynamicsPleasureAmount;
window.combatDynamicsMarkRelation = combatDynamicsMarkRelation;
window.combatDynamicsAcceptanceOverride = combatDynamicsAcceptanceOverride;
window.combatDynamicsRecordProposal = combatDynamicsRecordProposal;
window.combatDynamicsDebugCommand = combatDynamicsDebugCommand;
window.combatDynamicsDebugSnapshot = combatDynamicsDebugSnapshot;
const combatAgencyRelationData = Object.freeze({
	agencyPenisToMouth: Object.freeze({
		id: "agencyPenisToMouth", origin: "ask", sourceActor: "npc", sourceBodypart: "penis", targetActor: "player", targetBodypart: "mouth", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyVaginaToMouth: Object.freeze({
		id: "agencyVaginaToMouth", origin: "ask", sourceActor: "npc", sourceBodypart: "vagina", targetActor: "player", targetBodypart: "mouth", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyMouthToVagina: Object.freeze({
		id: "agencyMouthToVagina", origin: "ask", sourceActor: "npc", sourceBodypart: "mouth", targetActor: "player", targetBodypart: "vagina", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyMouthToPenis: Object.freeze({
		id: "agencyMouthToPenis", origin: "ask", sourceActor: "npc", sourceBodypart: "mouth", targetActor: "player", targetBodypart: "penis", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyMouthToAnus: Object.freeze({
		id: "agencyMouthToAnus", origin: "ask", sourceActor: "npc", sourceBodypart: "mouth", targetActor: "player", targetBodypart: "anus", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyHandToVagina: Object.freeze({
		id: "agencyHandToVagina", origin: "ask", sourceActor: "npc", sourceBodypart: "hand", targetActor: "player", targetBodypart: "vagina", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyHandToPenis: Object.freeze({
		id: "agencyHandToPenis", origin: "ask", sourceActor: "npc", sourceBodypart: "hand", targetActor: "player", targetBodypart: "penis", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyHandToAnus: Object.freeze({
		id: "agencyHandToAnus", origin: "ask", sourceActor: "npc", sourceBodypart: "hand", targetActor: "player", targetBodypart: "anus", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyPenisToVagina: Object.freeze({
		id: "agencyPenisToVagina", origin: "ask", sourceActor: "npc", sourceBodypart: "penis", targetActor: "player", targetBodypart: "vagina", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyPenisToAnus: Object.freeze({
		id: "agencyPenisToAnus", origin: "ask", sourceActor: "npc", sourceBodypart: "penis", targetActor: "player", targetBodypart: "anus", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	agencyPenisRelease: Object.freeze({
		id: "agencyPenisRelease", origin: "ask", sourceActor: "npc", sourceBodypart: "penis", targetActor: "player", targetBodypart: "control", transition: "combatAgencyTransition", ui: "combatAgencyAskLabel", availability: "combatAgencyCanTransition", acceptance: "combatSkillCheck", result: "success|refusal|invalid"
	}),
	directLeftHandToPenis: Object.freeze({
		id: "directLeftHandToPenis", origin: "direct", sourceActor: "player", sourceBodypart: "left hand", targetActor: "npc", targetBodypart: "penis", action: "leftgrab", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	}),
	directRightHandToPenis: Object.freeze({
		id: "directRightHandToPenis", origin: "direct", sourceActor: "player", sourceBodypart: "right hand", targetActor: "npc", targetBodypart: "penis", action: "rightgrab", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	}),
	directLeftHandToVagina: Object.freeze({
		id: "directLeftHandToVagina", origin: "direct", sourceActor: "player", sourceBodypart: "left hand", targetActor: "npc", targetBodypart: "vagina", action: "leftplay", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	}),
	directRightHandToVagina: Object.freeze({
		id: "directRightHandToVagina", origin: "direct", sourceActor: "player", sourceBodypart: "right hand", targetActor: "npc", targetBodypart: "vagina", action: "rightplay", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	}),
	directLeftHandToPenisStroke: Object.freeze({
		id: "directLeftHandToPenisStroke", origin: "direct", sourceActor: "player", sourceBodypart: "left hand", targetActor: "npc", targetBodypart: "penis", action: "leftstroke", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	}),
	directRightHandToPenisStroke: Object.freeze({
		id: "directRightHandToPenisStroke", origin: "direct", sourceActor: "player", sourceBodypart: "right hand", targetActor: "npc", targetBodypart: "penis", action: "rightstroke", transition: "combatAgencyTransition", ui: "directAttempt", availability: "combatAgencyDirectAvailable", acceptance: "combatSkillCheck", result: "success|resistance|invalid"
	})
});

function combatAgencyRelation(id) {
	return combatAgencyRelationData[id] || null;
}


function combatAgencyIsAskRelation(id) {
	return !!combatAgencyRelationData[id] && combatAgencyRelationData[id].origin === "ask";
}function combatAgencySameTarget(left, right) {
	return left === right || (left !== undefined && right !== undefined && Number(left) === Number(right));
}

function combatAgencyNpc(target) {
	return V.NPCList && V.NPCList[target] ? V.NPCList[target] : null;
}
function combatAgencyAskTarget() {
 const candidates = [V.penistarget, V.mouthtarget, V.vaginatarget, V.anustarget, V.lefttarget, V.righttarget];
 for (const candidate of candidates) {
  if (combatAgencyNpc(candidate)) return candidate;
 }
 return undefined;
}

function combatAgencyFree(value) {
	return value === 0 || value === "0" || value === undefined || value === null;
}

function combatAgencyAnatomy(npc, part) {
	if (!npc) return false;
	if (part === "penis") {
		return typeof npc.penisdesc === "string" && npc.penisdesc !== "none" && npc.penisdesc !== "clothed";
	}
	if (part === "vagina" && typeof npc.vaginadesc === "string") {
		return npc.vaginadesc !== "none" && npc.vaginadesc !== "clothed";
	}
	const value = npc[part];
	return value !== undefined && value !== null && value !== "none" && value !== "clothed";
}

function combatAgencyFaceOpen() {
	const type = String(V.worn?.face?.type || "");
	return !type.includes("face_covering") && !type.includes("gag");
}

function combatAgencyPlayerHas(part) {
	if (part === "vagina") return V.player?.vaginaExist === true;
	if (part === "penis") return V.player?.penisExist === true;
	if (part === "anus") return V.settings?.analEnabled === true;
	return true;
}

function combatAgencyPlayerUncovered(part) {
	if (typeof playerChastity !== "function") return true;
	return !playerChastity(part);
}

function combatAgencyPositionAllows(id) {
	/* Match the native combat reachability gates used by these existing poses. */
	if (V.position === undefined && V.walltype === undefined) return true;
	if (id === "agencyPenisToMouth") return V.position !== "wall";
	if (id === "agencyVaginaToMouth") return V.position !== "wall" || V.walltype === "front";
	if (["agencyPenisToVagina", "agencyPenisToAnus"].includes(id)) return V.walltype !== "front";
	if (["agencyMouthToVagina", "agencyMouthToPenis", "agencyMouthToAnus"].includes(id)) return V.walltype !== "front";
	return true;
}

function combatAgencyPlayerPart(target, part) {
	const states = {
		mouth: ["mouthuse", "mouthtarget"],
		vagina: ["vaginause", "vaginatarget"],
		penis: ["penisuse", "penistarget"],
		anus: ["anususe", "anustarget"]
	};
	const state = states[part];
	return state && !combatAgencyFree(V[state[0]]) && combatAgencySameTarget(V[state[1]], target);
}

function combatAgencyPlayerPartReleasable(target, part) {
	if (!combatAgencyPlayerPart(target, part)) return false;
	const use = V[{ mouth: "mouthuse", vagina: "vaginause", penis: "penisuse", anus: "anususe" }[part]];
	if (part === "mouth") return ["penis", "othervagina", "breasts", "otheranus", "facesit"].includes(use);
	if (part === "vagina") return ["penis", "othermouth", "otherhand", "handentrance", 1].includes(use);
	if (part === "penis") return ["othermouth", "otherhand", "handentrance", 1].includes(use);
	if (part === "anus") return ["penis", "othermouth", "hand", "handentrance", "otherhand"].includes(use);
	return false;
}

function combatAgencyNormalizeNativeContactState() {
	if (V.consensual !== 1 || !Array.isArray(V.NPCList)) return;
	const clear = (useName, stateName, targetName) => {
		V[useName] = 0;
		V[stateName] = 0;
		V[targetName] = 0;
	};
	const hasNpcHand = (target, positions) => {
		const npc = combatAgencyNpc(target);
		return !!npc && [npc.lefthand, npc.righthand].some(hand => positions.includes(hand));
	};
	if (["hand", "handentrance"].includes(V.anususe) && ["hand", "handentrance"].includes(V.anusstate)) {
		const positions = V.anusstate === "hand" ? ["anus"] : ["anusentrance"];
		if (!hasNpcHand(V.anustarget, positions)) clear("anususe", "anusstate", "anustarget");
	}
	if ([1, "otherhand", "handentrance"].includes(V.vaginause) && ["otherhand", "otherhandentrance"].includes(V.vaginastate)) {
		if (!hasNpcHand(V.vaginatarget, ["vagina", "vaginaentrance"])) clear("vaginause", "vaginastate", "vaginatarget");
	}
	if ([1, "otherhand", "handentrance"].includes(V.penisuse) && ["otherhand", "otherhandentrance"].includes(V.penisstate)) {
		if (!hasNpcHand(V.penistarget, ["penis", "penisentrance"])) clear("penisuse", "penisstate", "penistarget");
	}
}

function combatAgencyPendingMouthContact(target) {
	const npc = combatAgencyNpc(target);
	if (!npc || !combatAgencySameTarget(V.mouthtarget, target) || !combatAgencyFree(V.mouthuse)) return false;
	if (!["entrance", "imminent"].includes(V.mouthstate)) return false;
	return [npc.penis, npc.vagina].some(value => String(value || "").includes("mouth"));
}

function combatAgencyContactOwned(target, value) {
	if (combatAgencyFree(value)) return true;
	const text = String(value);
	if (text.includes("mouth") && (combatAgencyPlayerPart(target, "mouth") || combatAgencyPendingMouthContact(target))) return true;
	if (text.includes("vagina") && combatAgencyPlayerPart(target, "vagina")) return true;
	if (text.includes("anus") && combatAgencyPlayerPart(target, "anus")) return true;
	if (value === "facesit" && combatAgencyPlayerPart(target, "mouth")) return true;
	if (text === "othermouth" && (combatAgencyPlayerPart(target, "penis") || combatAgencyPlayerPart(target, "vagina"))) return true;
	if (value === "leftarm" && combatAgencySameTarget(V.lefttarget, target) && ["penis", "othervagina"].includes(V.leftarm)) return true;
	if (value === "rightarm" && combatAgencySameTarget(V.righttarget, target) && ["penis", "othervagina"].includes(V.rightarm)) return true;
	return false;
}

function combatAgencyNpcPartCanMakeRoom(target, part) {
	const npc = combatAgencyNpc(target);
	if (!npc) return false;
	return combatAgencyFree(npc[part]) || combatAgencyContactOwned(target, npc[part]);
}

function combatAgencyNpcLocationCanMakeRoom(target, locationPart) {
	const npc = combatAgencyNpc(target);
	if (!npc) return false;
	const value = npc.location?.[locationPart];
	if (combatAgencyFree(value)) return true;
	if (locationPart === "head") return combatAgencyNpcPartCanMakeRoom(target, "mouth");
	if (locationPart === "genitals") {
		const occupiedOrgans = [npc.penis, npc.vagina].filter(part => !combatAgencyFree(part));
		return occupiedOrgans.length > 0 && occupiedOrgans.some(part => combatAgencyContactOwned(target, part));
	}
	return false;
}

function combatAgencyNpcHandCanMakeRoom(target) {
	const npc = combatAgencyNpc(target);
	return !!npc && (combatAgencyFree(npc.lefthand) || combatAgencyContactOwned(target, npc.lefthand) || combatAgencyFree(npc.righthand) || combatAgencyContactOwned(target, npc.righthand));
}

function combatAgencyNpcGenitalCanMakeRoom(target) {
	const npc = combatAgencyNpc(target);
	return !!npc && (combatAgencyNpcPartCanMakeRoom(target, "vagina") || combatAgencyNpcPartCanMakeRoom(target, "penis") || npc.vagina === "facesit");
}

function combatAgencyDirectAvailable(id, target) {
	const npc = combatAgencyNpc(target);
	const relation = combatAgencyRelation(id);
	if (!relation || relation.origin !== "direct" || V.consensual !== 1 || V.enemytype !== "man" || !npc) return false;
	if (["directLeftHandToPenis", "directLeftHandToPenisStroke"].includes(id)) return combatAgencyFree(V.leftarm) && npc.penis === 0;
	if (["directRightHandToPenis", "directRightHandToPenisStroke"].includes(id)) return combatAgencyFree(V.rightarm) && npc.penis === 0;
	if (id === "directLeftHandToVagina") return combatAgencyFree(V.leftarm) && npc.vagina === 0;
	if (id === "directRightHandToVagina") return combatAgencyFree(V.rightarm) && npc.vagina === 0;
	return false;
}

function combatAgencyAskPromiscuityAllowed(id) {
	const required = {
		agencyMouthToVagina: 1,
		agencyMouthToPenis: 1,
		agencyHandToVagina: 0,
		agencyHandToPenis: 0,
		agencyPenisToVagina: 2,
		agencyHandToAnus: 2,
		agencyPenisToAnus: 4,
		agencyMouthToAnus: 3
	}[id];
	if (required === undefined || required === 0 || V.promiscuityIgnore) return true;
	return typeof hasSexStat === "function" && hasSexStat("promiscuity", required);
}

function combatAgencyCanTransition(id, target) {
	const relation = combatAgencyRelation(id);
	const npc = combatAgencyNpc(target);
	if (!relation || !npc || V.consensual !== 1 || V.enemytype !== "man" || V.npcSub || V.gloryhole) return false;
	if (relation.origin === "ask" && !combatAgencyAskPromiscuityAllowed(id)) return false;
	if (relation.origin === "direct") return combatAgencyDirectAvailable(id, target);
	if (!combatAgencyPositionAllows(id)) return false;
	switch (id) {
		case "agencyPenisToMouth":
			return V.mouthuse === 0 && combatAgencyFaceOpen() && combatAgencyAnatomy(npc, "penis");
		case "agencyPenisToVagina":
			return combatAgencyPlayerHas("vagina") && combatAgencyPlayerUncovered("vagina") && (!combatAgencyPlayerPart(target, "vagina") || combatAgencyPlayerPartReleasable(target, "vagina")) && V.walltype !== "front" && V.bottomuse === 0 && combatAgencyAnatomy(npc, "penis");
		case "agencyPenisToAnus":
			return combatAgencyPlayerHas("anus") && combatAgencyPlayerUncovered("anus") && (!combatAgencyPlayerPart(target, "anus") || combatAgencyPlayerPartReleasable(target, "anus")) && V.walltype !== "front" && V.settings?.analEnabled === true && V.bottomuse === 0 && combatAgencyAnatomy(npc, "penis");
		case "agencyPenisRelease":
			return !combatAgencyFree(npc.penis) && combatAgencyAnatomy(npc, "penis");
		case "agencyVaginaToMouth":
			return V.mouthuse === 0 && combatAgencyFaceOpen() && combatAgencyAnatomy(npc, "vagina");
		case "agencyMouthToVagina":
			return combatAgencyPlayerHas("vagina") && combatAgencyPlayerUncovered("vagina") && (!combatAgencyPlayerPart(target, "vagina") || combatAgencyPlayerPartReleasable(target, "vagina"));
		case "agencyMouthToPenis":
			return combatAgencyPlayerHas("penis") && combatAgencyPlayerUncovered("penis") && (!combatAgencyPlayerPart(target, "penis") || combatAgencyPlayerPartReleasable(target, "penis"));
		case "agencyMouthToAnus":
			return combatAgencyPlayerHas("anus") && combatAgencyPlayerUncovered("anus") && (!combatAgencyPlayerPart(target, "anus") || combatAgencyPlayerPartReleasable(target, "anus"));
		case "agencyHandToVagina":
			return combatAgencyPlayerHas("vagina") && combatAgencyPlayerUncovered("vagina") && (!combatAgencyPlayerPart(target, "vagina") || combatAgencyPlayerPartReleasable(target, "vagina"));
		case "agencyHandToPenis":
			return combatAgencyPlayerHas("penis") && combatAgencyPlayerUncovered("penis") && (!combatAgencyPlayerPart(target, "penis") || combatAgencyPlayerPartReleasable(target, "penis"));
		case "agencyHandToAnus":
			return combatAgencyPlayerHas("anus") && combatAgencyPlayerUncovered("anus") && (!combatAgencyPlayerPart(target, "anus") || combatAgencyPlayerPartReleasable(target, "anus"));
		default:
			return false;
	}
...
function combatAgencyAskRelationIds(target) {
	if (V.consensual !== 1 || V.enemytype !== "man" || V.npcSub || V.gloryhole) return [];
	return Object.keys(combatAgencyRelationData).filter(id => combatAgencyRelationData[id].origin === "ask" && combatAgencyCanTransition(id, target));
}

function combatAgencyAskLabel(id, target) {
	const npc = combatAgencyNpc(target);
	if (!npc) return "invalid request";
	const his = npc.pronouns?.his || "their";
	switch (id) {
		case "agencyPenisToMouth": return "to move " + his + " penis to your mouth";
		case "agencyPenisToVagina": return "to move " + his + " penis to your pussy";
		case "agencyPenisToAnus": return "to move " + his + " penis to your anus";
		case "agencyPenisRelease": return "to let you take over";
		case "agencyVaginaToMouth": return "to move " + his + " pussy to your mouth";
		case "agencyMouthToVagina": return "to use " + his + " mouth on your pussy";
		case "agencyHandToVagina": return "to use " + his + " hand on your pussy";
		case "agencyMouthToPenis": return "to use " + his + " mouth on your penis";
		case "agencyHandToPenis": return "to use " + his + " hand on your penis";
		case "agencyMouthToAnus": return "to use " + his + " mouth on your anus";
		case "agencyHandToAnus": return "to use " + his + " hand on your anus";
		default: return "invalid request";
	}
}

function combatAgencyDesireScore(target, id) {
	const relation = combatAgencyRelation(id);
	const state = typeof combatDynamicsState === "function" ? combatDynamicsState() : null;
	const profile = state?.profiles?.[Number(target)] || null;
	if (!relation || !profile?.candidates) return 20;
	const sourceSex = relation.sourceBodypart === "penis" ? "penis" : relation.sourceBodypart === "vagina" ? "vagina" : null;
	const className = combatDynamicsClassForRelation(id);
	const targetName = relation.targetBodypart === "mouth" ? "mouth" : relation.targetBodypart === "vagina" ? "vagina" : relation.targetBodypart === "anus" ? "anus" : relation.targetBodypart === "penis" ? "penis" : relation.targetBodypart === "control" ? null : "hands";
	const matching = profile.candidates.filter(candidate => (!sourceSex || candidate.sourceSex === sourceSex) && candidate.className === className);
	const exact = matching.find(candidate => !targetName || candidate.target === targetName);
	const candidate = exact || matching[0];
	return combatDynamicsClamp(candidate?.score ?? 20, 0, 40);
}

function combatAgencySuccessChance(target, id) {
	const relation = combatAgencyRelation(id);
	if (!combatAgencyCanTransition(id, target)) return null;
	if (V.role === "islander" && (!V.islander_language || V.islander_language < 100)) return 0;
	/* Custom agency formula inputs, clamped to their native encounter bounds. */
	const trust = combatDynamicsClamp(Number(V.enemytrust) || 0, -100, 100);
	const anger = combatDynamicsClamp(Number(V.enemyanger) || 0, 0, 200);
	const arousal = combatDynamicsClamp(Number(V.enemyarousal) || 0, 0, 500);
	const desire = id === "agencyPenisRelease" ? 20 : combatAgencyDesireScore(target, id);
	const seduction = combatDynamicsClamp(Number(currentSkillValue("seductionskill")) || 0, 0, 1000);
	const O = (
		0.70 * ((trust + 100) / 200) +
		0.20 * ((desire - 20) / 20) +
		0.20 * (arousal / 500) +
		0.50 * (seduction / 1000)
	) * (1 - Math.pow(anger / 200, 2));
	return combatDynamicsClamp(Math.floor(O * 100), 0, 100);
}

function combatAgencyAcceptanceCheck(target, id) {
	const chance = combatAgencySuccessChance(target, id);
	if (chance === null) return false;
	if (V.gamemode === "soft") return true;
	return Math.random() * 100 < chance;
}

function combatAgencyDifficultyText(target, id) {
	if (id === "agencyPenisRelease") return "";
	const chance = combatAgencySuccessChance(target, id);
	if (chance === null) return " (Unavailable)";
	if (chance >= 100) return " (Very Easy)";
	if (chance >= 80) return " (Easy)";
	if (chance >= 60) return " (Medium)";
	if (chance >= 40) return " (Challenging)";
	if (chance >= 20) return " (Hard)";
	if (chance >= 1) return " (Very Hard)";
	return " (Impossible)";
}

function combatAgencyRequestIsValid(request, target) {
	if (!request || request.origin !== "ask" || combatAgencyRelation(request.relation)?.origin !== "ask") return false;
	if (!combatAgencySameTarget(request.target, target) || !combatAgencySameTarget(request.target, combatAgencyAskTarget()) || !combatAgencyNpc(target) || V.consensual !== 1) return false;
	if (typeof State !== "undefined" && request.turn !== undefined && request.turn !== State.activeIndex) return false;
	for (const stateName of ["mouthuse", "vaginause", "penisuse", "anususe"]) {
		if (request.originState?.[stateName] !== undefined && request.originState[stateName] !== V[stateName]) return false;
	}
	return true;
}

window.combatAgencyRelations = combatAgencyRelationData;
window.combatAgencyRelation = combatAgencyRelation;
window.combatAgencyIsAskRelation = combatAgencyIsAskRelation;
window.combatAgencyCanTransition = combatAgencyCanTransition;
window.combatAgencyDirectAvailable = combatAgencyDirectAvailable;
window.combatAgencyPendingMouthContact = combatAgencyPendingMouthContact;
window.combatAgencyNormalizeNativeContactState = combatAgencyNormalizeNativeContactState;
window.combatAgencyAskTarget = combatAgencyAskTarget;
window.combatAgencyAskRelationIds = combatAgencyAskRelationIds;
window.combatAgencyAskLabel = combatAgencyAskLabel;
window.combatAgencySuccessChance = combatAgencySuccessChance; window.combatAgencyAcceptanceCheck = combatAgencyAcceptanceCheck;
window.combatAgencyDifficultyText = combatAgencyDifficultyText;
window.combatAgencyRequestIsValid = combatAgencyRequestIsValid;

function hairdressersReset() {
	$(() =>
		$("#hairDressers").off("change.dolHairdressers", ".macro-listbox, .macro-radiobutton, .macro-checkbox").on("change.dolHairdressers", ".macro-listbox, .macro-radiobutton, .macro-checkbox", function (e) {
...
function hairdressersResetAlt() {
	$(() =>
		$("#hairDressersSydney").off("click.dolHairdressersSydney", ".macro-cycle").on("click.dolHairdressersSydney", ".macro-cycle", function (e) {
...
function browsDyeReset() {
	$(() =>
		jQuery(document).off("change.dolBrowsDye", "#listbox-browsdyeoption").on("change.dolBrowsDye", "#listbox-browsdyeoption", function (e) {
...
function NPCSettingsReset() {
	jQuery(".passage").off("change.dolNPCSettings", "#listbox--npcid").on("change.dolNPCSettings", "#listbox--npcid", function (e) {
...
function loveInterestFunction() {
	jQuery(document).off("change.dolLoveInterest", "#listbox-loveinterestprimary").on("change.dolLoveInterest", "#listbox-loveinterestprimary", function (e) {
		Wikifier.wikifyEval("<<replace #loveInterest>><<loveInterest>><</replace>>");
	});
	jQuery(document).off("change.dolLoveInterest", "#listbox-loveinterestsecondary").on("change.dolLoveInterest", "#listbox-loveinterestsecondary", function (e) {
		Wikifier.wikifyEval("<<replace #loveInterest>><<loveInterest>><</replace>>");
	});
	jQuery(document).off("change.dolLoveInterest", "#listbox-loveinteresttertiary").on("change.dolLoveInterest", "#listbox-loveinteresttertiary", function (e) {
		Wikifier.wikifyEval("<<replace #loveInterest>><<loveInterest>><</replace>>");
	});
}
...
function cheatPregnancyNPCReset() {
	jQuery("#customOverlayContent").off("change.dolPregnancyNPC", "#listbox--pregnantnpcid").on("change.dolPregnancyNPC", "#listbox--pregnantnpcid", function (e) {
...
function featsPointsMenuReset() {
	jQuery(document).off("change.dolFeatsPoints", "#listbox--upgradenameid").on("change.dolFeatsPoints", "#listbox--upgradenameid", () => {
...
function startingPlayerImageReset() {
	jQuery(document).off("change.dolStartingPlayerImage", "#settingsDiv .macro-radiobutton,#settingsDiv ,#settingsDiv .macro-checkbox").on("change.dolStartingPlayerImage", "#settingsDiv .macro-radiobutton,#settingsDiv ,#settingsDiv .macro-checkbox", () => {
