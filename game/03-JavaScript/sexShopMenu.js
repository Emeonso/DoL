...
setup.sextoyFunctions = {
	...
	isWorn(name) {
		if (setup.sextoyFunctions.notExists(name)) return false;
		const items = V.player.inventory.sextoys[name];
		const setupToy = setup.sextoys.find(item => item.name === name);
		return items.some(item => item.worn) || (setupToy?.category === "strap-on" && V.worn.under_lower?.type?.includes("strap-on"));
	},
	unWear(name) {
...
setup.sextoys = [
	...
	},
	{
		...
		name_underscore: "strap_on_horse_cock",
		clothes_index: 34,
	...
	},
	{
		...
		name_underscore: "strap_on_knotted_cock",
		clothes_index: 35,
	...
	},
	{
		...
		name_underscore: "strap_on_dolphin_cock",
		clothes_index: 48,
	...
	},
	{
		...
		name_underscore: "strap_on_tentacle",
		clothes_index: 49,
...
function sexShopOnBuyClick(index, inSexShop = true, colour, costsMoney = true) {
	if (window.inventorySexToyCanBuy && !window.inventorySexToyCanBuy()) return false;
	const item = setup.sextoys[index];
	const icon = document.querySelector("#ssm_descContainer .ssm_icon.icon:not(.infront)");
