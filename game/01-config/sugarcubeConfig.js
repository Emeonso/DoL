...
Save.onLoad.add(function normalizeCombatAgencySave(save) {
	if (!save?.state?.history) return;
	for (const frame of save.state.history) {
		if (!frame.variables) continue;
		frame.variables.combatAgencyRequest = 0;
		frame.variables.combatAgencyHandled = 0;
		frame.variables.combatAgencyTransitionOk = 0;
		frame.variables.combatAgencyTransitionResult = "invalid";
	}
});
...
function onSave(save, details) {
	...
	// Portrait appearance is generated in the active state during rendering. Copy it
	// into the serialized current frame before the save is compressed.
	if (window.NamedNpcPortrait?.prepareSave) window.NamedNpcPortrait.prepareSave(save);
