...
const statChange = (() => {
	 ...
	 * Trauma ($trauma) has a maximum value of 50,000.
	 *
	...
	function trauma(amount, source) {
		...
		if (amount) {
			let traumaMod = 1;
			if (amount >= 0) {
				// Non-consensual assault impact is ten times more traumatic.
				if (V.consensual === 0) amount *= 10;
	...
	function updatePlayerTraumaState() {
		V.sleeptrouble = V.trauma >= V.traumamax / 5000 ? 1 : 0;
	...
	function dolWornTraitCounters() {
		const worn = V.worn;
		const slots = Object.keys(worn);
		const cached = T.dolWornTraitCache;
		if (
			cached?.worn === worn &&
			cached.slots.length === slots.length &&
			slots.every(slot => cached.items[slot] === worn[slot] && cached.types[slot] === worn[slot]?.type)
		) {
			return cached;
		}
		const result = { worn, slots, items: {}, types: {}, fetish: 0, dark: 0, holy: 0 };
		for (const slot of slots) {
			const item = worn[slot];
			const type = item?.type || "";
			result.items[slot] = item;
			result.types[slot] = type;
			if (type.includes("fetish")) result.fetish++;
			if (type.includes("dark")) result.dark++;
			if (type.includes("holy")) result.holy++;
		}
		T.dolWornTraitCache = result;
		return result;
	}
	...
	function minArousal() {
		...
		result += dolWornTraitCounters().fetish * 150;
	...
	function spray(amount) {
		...
		amount = Number(amount);
		if (window.inventoryPepperSprayAdjust && !V.infinitespray) {
			if (amount < 0 && V.prof.spray >= random(1, 5000)) {
				V.effectsmessage = 1;
				V.prof_spray_message = 1;
			} else {
				window.inventoryPepperSprayAdjust(amount, true);
				if (amount < 0) prof("spray", amount * -5);
			}
			return;
		}
		if (amount > 0) V.spray += amount;
		else if (amount && !V.infinitespray) V.spray += amount;
	...
	function awareness(amount) {
		...
		amount = Number(amount);
		if (amount) {
			const mod = dolWornTraitCounters().dark + 1;
	...
	function purity(amount) {
		...
		amount = Number(amount);
		if (amount) {
			const mod = dolWornTraitCounters().holy + 1;
