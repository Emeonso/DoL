...
function apparentbottomsizecheck() {
	...
	if (V.worn.over_lower.rearresize != null) {
...
function hygieneClamp() {
	const value = Number(V.hygiene);
	V.hygiene = Math.clamp(Number.isFinite(value) ? value : 0, 0, 3000);
	return V.hygiene;
}
DefineMacro("hygieneClamp", hygieneClamp);
...
function calculateallure() {
	...
	/* physique, breast size, bottom size, and temporary presentation */
	const physiqueAttractiveness = {
		f: [-500, -250, 0, 500, 250, 0, 0],
		h: [-1000, -500, 0, 500, 250, 0, 0],
		m: [-1000, -667, -333, 0, 333, 667, 1000],
	};
	const physiqueLevel = V.physiquesize > 0 ? Math.clamp(Math.floor((V.physique / V.physiquesize) * 7), 0, 6) : 0;
	attractiveness += (physiqueAttractiveness[V.player.sex] || physiqueAttractiveness.f)[physiqueLevel];

	apparentbreastsizecheck();
	const breastAttractiveness = [0, 30, 65, 95, 190, 220, 315, 345, 405, 425, 445, 470, 500];
	attractiveness += breastAttractiveness[Math.clamp(V.player.perceived_breastsize, 0, breastAttractiveness.length - 1)];

	apparentbottomsizecheck();
	const bottomAttractiveness = [0, 12, 24, 51, 79, 94, 106, 114, 125];
	attractiveness += bottomAttractiveness[Math.clamp(V.player.perceived_bottomsize, 0, bottomAttractiveness.length - 1)];

	// Tanning is continuous and reversible, capped at +250.
	attractiveness += Math.floor(Math.clamp(Skin.totalTan ?? 0, 0, 100) * 2.5);
	/* tf bonuses */
	...
	for (const makeup of ["lipstick", "mascara", "eyeshadow", "blusher"]) {
		if (V.makeup[makeup]) attractiveness += 250;
	}
	if (
		(V.haircolour && V.haircolour !== V.naturalhaircolour) ||
		(V.hairfringecolour && V.hairfringecolour !== V.naturalhaircolour)
	) {
		attractiveness += 250;
	}
	if (painToTearsLvl(V.pain) >= 3) attractiveness -= 1000;
	attractiveness -= V.hygiene;
