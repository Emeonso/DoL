...
function wardrobeIsComboLowerLocked(worn, slot) {
	if (slot !== "lower" || !worn?.upper || !worn?.lower) return false;

	const upper = worn.upper;
	const lower = worn.lower;
	const lowerName = upper.outfitPrimary?.lower;

	return (
		upper.name && upper.name !== "naked" &&
		lower.name && lower.name !== "naked" &&
		lowerName && !["broken", "split"].includes(lowerName) &&
		lower.name === lowerName &&
		Array.isArray(lower.outfitSecondary) &&
		lower.outfitSecondary[0] === "upper" &&
		lower.outfitSecondary[1] === upper.name
	);
}
