...
function wardrobeIsComboLowerLocked(worn, slot) {
	if (slot !== "lower" || !worn?.upper) return false;

	const upper = worn.upper;
	const lowerName = upper.outfitPrimary?.lower;

	return (
		upper.name && upper.name !== "naked" &&
		lowerName && !["broken", "split"].includes(lowerName)
	);
}
window.wardrobeIsComboLowerLocked = wardrobeIsComboLowerLocked;

function wardrobeFindComboLower(primary, wornLower, wardrobeLower) {
	const lowerName = primary?.outfitPrimary?.lower;
	const result = { combo: false, source: "none", item: null, index: -1 };
	if (!lowerName || ["broken", "split"].includes(lowerName)) return result;

	result.combo = true;
	const matchesPrimary = secondary => {
		if (!secondary || secondary.name !== lowerName) return false;
		if (!Array.isArray(secondary.outfitSecondary) || secondary.outfitSecondary[0] !== "upper" || secondary.outfitSecondary[1] !== primary.name) return false;
